/**
 * Simulates a single year of financial activity based on the provided parameters
 * @param {Object} params - The parameters for the simulation
 * @returns {Object} - The updated state after processing the year
 */
function simulateYear(params) {
    // Clone the state to avoid modifying the input
    const state = JSON.parse(JSON.stringify(params.state));
    ...
    for (const investment of investments) {
      ...
      const expenses = avgValue * investment.expenseRatio;
  ...
  function adjustTaxBrackets(taxBrackets, inflationRate) {
    ...
    adjusted.federal = adjusted.federal.map(bracket => ({
      ...
      upperBound: bracket.upperBound * (1 + inflationRate)
    }));
  ...
  function calculateIncomeTax(brackets, income) {
    ...
    for (let i = 0; i < brackets.length; i++) {
      ...
      const taxableInThisBracket = Math.min(
        bracket.upperBound - prevUpperBound,
  ...
  function calculatePurchaseAmounts(assetAllocation, totalAmount) {
    ...
    for (const [investmentId, percentage] of Object.entries(assetAllocation)) {
      const amount = totalAmount * (percentage / 100);
      
      if (amount > 0) {
        purchases.push({
          investmentId,
          investmentType: assetAllocation[investmentId].type,
          taxStatus: assetAllocation[investmentId].taxStatus,
          amount
        });
      }
    }
  ...
  /**
   * Test the simulation with hard-coded values
   */
  function testSimulation() {
    ...
    const testParams = {
      ...
      rmdTable: {
        ...
        79: 21.1,
        80: 20.2,
        // Add more age-based values as needed
      },
    ...
    // Run the simulation
    console.log("Starting test simulation for year 2025...");
    const result = simulateYear(testParams);
    
    // Display results
    console.log("Simulation Results:");
    console.log("--------------------");
    console.log(`Inflation Rate: ${(result.inflationRate * 100).toFixed(2)}%`);
    console.log(`Total Assets: $${result.totalAssets.toFixed(2)}`);
    console.log(`Income: $${result.curYearIncome.toFixed(2)}`);
    console.log(`Social Security: $${result.curYearSS.toFixed(2)}`);
    console.log(`Capital Gains: $${result.curYearGains.toFixed(2)}`);
    console.log(`Early Withdrawals: $${result.curYearEarlyWithdrawals.toFixed(2)}`);
    
    console.log("\nInvestment Details:");
    console.log("--------------------");
    result.investments.forEach(inv => {
      console.log(`${inv.investmentType} (${inv.taxStatus}): $${inv.value.toFixed(2)}`);
    });
  ...
  /**
   * Run multiple simulations in parallel and analyze the results
   * @param {Object} baseParams - Base parameters for the simulation
   * @param {Number} numSimulations - Number of simulations to run
   * @param {Number} numYears - Number of years to simulate
   * @param {Function} successCriteriaFn - Function to determine if a simulation succeeded
   * @returns {Object} - Results and statistics from the simulations
   */
  async function runSimulations(baseParams, numSimulations = 100, numYears = 30, successCriteriaFn = null) {
    console.log(`Starting ${numSimulations} simulations over ${numYears} years...`);
    
    // Default success criteria: final assets > financial goal
    const isSuccessful = successCriteriaFn || ((finalState) => 
      finalState.totalAssets >= baseParams.financialGoal
    );
    
    // Create array of simulation promises
    const simulationPromises = Array.from({ length: numSimulations }, (_, i) => {
      return runOneSimulation(baseParams, numYears, i);
    });
    ...
    const successfulSimulations = simulationResults.filter(result => 
      isSuccessful(result.finalState)
    );
    
    const successPercentage = (successfulSimulations.length / numSimulations) * 100;
    
    // Prepare data for visualization
    const visualizationData = prepareVisualizationData(simulationResults, successfulSimulations);
   ...
   * @returns {Object} - Results from this simulation
   */
  async function runOneSimulation(baseParams, numYears, simulationIndex) {
    ...
    // Add some randomness to initial parameters for Monte Carlo simulation
  ...
  /**
   * Prepare data for visualization
   * @param {Array} allResults - Results from all simulations
   * @param {Array} successfulResults - Results from successful simulations
   * @returns {Object} - Data for visualization
   */
  function prepareVisualizationData(allResults, successfulResults) {
    // Extract data for final asset distribution
    const finalAssetValues = allResults.map(result => result.finalState.totalAssets);
    
    // Extract yearly data for time series visualization
    const timeSeriesData = {};
    
    // Group by successful/unsuccessful
    const successfulIndices = new Set(successfulResults.map(result => result.simulationIndex));
    
    // Process each simulation
    allResults.forEach(simulation => {
      const isSuccess = successfulIndices.has(simulation.simulationIndex);
      const category = isSuccess ? 'Successful' : 'Unsuccessful';
      
      if (!timeSeriesData[category]) {
        timeSeriesData[category] = [];
      }
      
      // Add this simulation's data
      timeSeriesData[category].push({
        simulationIndex: simulation.simulationIndex,
        yearlyData: simulation.yearlyResults
      });
    });
  ...
  function createVisualizations(data) {
    ...
    Object.entries(data.timeSeriesData).forEach(([category, simulations]) => {
      ...
      const yearlyStats = years.map((year, yearIndex) => {
        const assetValues = simulations.map(sim => sim.yearlyData[yearIndex].totalAssets);
        
        // Sort values for percentile calculations
        assetValues.sort((a, b) => a - b);
        
        // Calculate percentiles
        const median = assetValues[Math.floor(assetValues.length / 2)];
        const p10 = assetValues[Math.floor(assetValues.length * 0.1)];
        const p90 = assetValues[Math.floor(assetValues.length * 0.9)];
    ...
    // Return configurations (in a real implementation, we would render these)
    return {
  ...
  /**
   * Create HTML with embedded Plotly visualizations
   * @param {Object} visualizationConfig - Plotly configurations
   * @param {Number} successPercentage - Percentage of successful simulations
